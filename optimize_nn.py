...
def load_graphs_from_hdf5(filename, max_samples=50):
    """
    Load a limited number of graphs from the HDF5 file for testing purposes.
    ...
    with h5py.File(filename, 'r') as f:
        keys = list(f.keys())[:max_samples]  # Limit to max_samples keys
        for key in keys:
            pickled_graph = f[key][()]
